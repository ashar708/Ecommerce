  ...
  return (
    <div className='container mx-auto px-4 my-6 relative'>
        ...
        <button className='bg-white shadow-md rounded-full p-1 absolute right-0 text-lg hidden md:block' onClick={scrollRight}><FaAngleRight/></button>
        {
            ...
            data.map((prod,idx)=>(
                <Link to={"product/"+prod?._id} className='w-full min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px] h-36 bg-white rounded-sm shadow flex'>
                    <div className='bg-slate-200 h-full p-4 min-w-[120px] md:min-w-[145px]'>
                        <img src={prod.productImage[0]} className='h-full object-scale-down hover:scale-110 transition-all'/>
                    </div>
                    <div className='py-4 px-4 grid'>
                        ...
                        <button onClick={(e)=>handleAddToCart(e,prod?._id)} className='text-sm bg-red-500 hover:bg-red-700 text-white px-3 py-0.5 rounded-full'>Add to Cart</button>
                    </div>
                </Link>
            ))
        )
